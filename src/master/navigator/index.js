...
export class Navigator {
  ...
  pushInitSlave(initParams) {
    ...
    // slave的init调用
    this.initSlave.init(initParams).then(initRes => {
  ...
  redirectTo(params) {
    params.url = this.resolvePathByTopSlave(params.url);
    this.preCheckPageExist(params.url);
    const normalPageStack = this.history.historyStack.filter(
      slave => !slave.isClosing
    );
  ...
  switchTab(params) {
    params.url = this.resolvePathByTopSlave(params.url);
    return this.initSlave.switchTab(params).then(res => {
  ...
  reLaunch(params = {}) {
    ...
    return targetSlave.reLaunch(params).then(res => {
      const slaveId = res.wvID;
  ...
  resolvePathByTopSlave(path) {
    ...
    const topSlaveUri = this.history
      .getTopSlaves()[0]
      .getUri()
      .replace(/[^\/]*$/g, "");
